...
import com.xiaxiao.chinatime.bean.Article;
import com.xiaxiao.chinatime.bean.ChaoDai;
import com.xiaxiao.chinatime.bean.MyUser;
import com.xiaxiao.chinatime.bean.Year;
import com.xiaxiao.chinatime.bean.YearEvent;
import com.xiaxiao.chinatime.util.UIDialog;
...
public class BmobServer {
    ...
    public BmobServer(Context context) {
...
///////////////////////////////////////////////////////////////China Time/////////////////////////////////////
    public void addChaodai(final ChaoDai chaoDai,SuccessListener successListener) {
        showWaitDialog();
        addListener(successListener);
        chaoDai.save(new SaveListener<String>() {
            @Override
            public void done(String objectId, BmobException e) {
                dismissWaitDialog();
                if (e == null) {
                    handleSuccess(chaoDai);
                } else {
                    handleError(e);
                }
    ...
    public void getChaodais(final SuccessListener successListener) {
        addListener(successListener);
        BmobQuery<ChaoDai> query = new BmobQuery<>();
        query.order("createdAt");
        query.findObjects(new FindListener<ChaoDai>() {
            @Override
            public void done(List<ChaoDai> list, BmobException e) {
                dismissWaitDialog();
                if (e==null) {
    ...
    public void getYears(final SuccessListener<List<Year>> successListener) {
        addListener(successListener);
        BmobQuery<Year> query = new BmobQuery<>();
        query.order("year");
        if (this.mPage!=null) {
            if (mPage.isLastPage) {
                return;
            }
            query.setLimit(this.mPage.getPageSize());
            query.setSkip(this.mPage.getPageSize() * mPage.currentPage);
        }
        query.findObjects(new FindListener<Year>() {
            @Override
            public void done(List<Year> list, BmobException e) {
                dismissWaitDialog();
                if (e==null) {
                    successListener.onSuccess(list);
                    if (mPage!=null) {
                        if (list.size() < mPage.getPageSize()) {
                            mPage.isLastPage = true;
                        } else {
                            mPage.next();
                        }
                    }
    ...
    public void addYearEvent(final YearEvent yearEvent, BmobListener<YearEvent> successListener) {
        showWaitDialog();
        addListener(successListener);
        yearEvent.save(new SaveListener<String>() {
            @Override
            public void done(String objectId, BmobException e) {
                dismissWaitDialog();
                if (e == null) {
                    handleSuccess(yearEvent);
                } else {
                    handleError(e);
                }
    ...
    public void getYear(int year, final SuccessListener<Year> successListener) {
        showWaitDialog();
        addListener(successListener);
        BmobQuery<Year> query = new BmobQuery<>();
        query.addWhereEqualTo("year", year);
//        query.include("yearEvent");
        query.findObjects(new FindListener<Year>() {
            @Override
            public void done(List<Year> list, BmobException e) {
                dismissWaitDialog();
                if (e==null) {
                    successListener.onSuccess(list.get(0));
                }
            }
        });
    }

    public void updateYear(final Year year, final SuccessListener<Year> successListener) {
        addListener(successListener);
        showWaitDialog();
        year.update(new UpdateListener() {
            @Override
            public void done(BmobException e) {
                dismissWaitDialog();
                if (e==null) {
                    successListener.onSuccess(year);
                }
            }
        });
    }

    public void getYearEvents(int year, final SuccessListener<List<YearEvent>> successListener) {
        addListener(successListener);
        showWaitDialog();
        BmobQuery<YearEvent> query = new BmobQuery<>();
        query.addWhereEqualTo("year", year);
        query.findObjects(new FindListener<YearEvent>() {
            @Override
            public void done(List<YearEvent> list, BmobException e) {
                dismissWaitDialog();
                if (e==null) {
                    successListener.onSuccess(list);
                }
